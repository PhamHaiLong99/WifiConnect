...
public final class WifiConnectionReceiver extends BroadcastReceiver {
    ...
    public WifiConnectionReceiver(@NonNull final WifiConnectionCallback callback,
    ...
    public void onReceive(final Context context, @NonNull final Intent intent) {
        ...
        if (isAndroidQOrLater()) {
            if (Objects.equals(WifiManager.ACTION_WIFI_NETWORK_SUGGESTION_POST_CONNECTION, action)) {
                ...
                wifiLog("Connection Broadcast state: in ACTION_WIFI_NETWORK_SUGGESTION_POST_CONNECTION " + state);
            ...
            if (Objects.equals(WifiManager.SUPPLICANT_STATE_CHANGED_ACTION, action)) {
                final SupplicantState state = intent.getParcelableExtra(WifiManager.EXTRA_NEW_STATE);
                final int suppl_error = intent.getIntExtra(WifiManager.EXTRA_SUPPLICANT_ERROR, -1);
                wifiLog("Connection Broadcast state: in SUPPLICANT_STATE_CHANGED_ACTION " + state);
                wifiLog("suppl_error: " + suppl_error);
                if (mScanResult == null && isAlreadyConnected2(mWifiManager, ssid)) {
                    mWifiConnectionCallback.successfulConnect();
                }
